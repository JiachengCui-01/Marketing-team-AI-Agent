...
- **自动化入口（定时跑的分析任务）**：三个平级标签 —— ① **行业简报**：配置主题、详略、推送时间与时区，定时抓取并生成**分级来源**摘要；② **全盘发现**：开头是 **AI 选品总结与建议** —— 本期把哪几条「类目＋外观组合」放进开发、目标价格带、物理包络、要在设计上解决的缺陷、为什么是现在、什么会让它死掉，以及哪几条别碰；每条都能点开背后的证据。再往下是家具部门 12 个追踪子类目的机会看板（确定性机会分 + 分解 + 证据覆盖度）、涨跌品类、机会象限（每个点是「大致品类＋细分品类＋颜色＋外观元素」的一条产品线，横轴是该组合内前三大品牌拿走之后还剩多少，纵轴是它占本类目头部销售额的份额比上月升降了几个百分点、再减去本图中位 —— 对着 0 画时整月的稀释会把所有点压到线下，四个角空掉两个）、价格带结构、新品可行性、退货率对同级类目均值；③ **品类深度**：手选一个类目做深研 —— 关键词版图、竞品 ASIN、评论痛点、流量结构、产品机会卡，并可一键生成**产品定义书（PRD）**。三者都支持「立即刷新」。市场分析**没有兜底路径** —— 拿不到数据时报数据缺口，而不是给一个猜出来的推荐。
...
- **产品图 AI 生成**：文生图（Gemini）、上传参考图、一键抠图去背景、按渠道风格（Amazon 合规白底 1:1 / Wayfair 列表图 1:1 / 独立站 Hero 16:9 / Instagram 4:5 / Pinterest 2:3）生成，或套模板画布合成。模板覆盖家具最需要的图型：**尺寸标注图、材质工艺特写、房间实景、尺度对比**。
  每个渠道风格自带三层规格：**平台硬规则**（如 Amazon 主图纯白 RGB 255,255,255、仅本体、无文字、占比 85%）、**艺术指导**（构图/布光，用户需求可覆盖）、**交付质量**（真实照片质感、结构件数正确、几何不畸变、无文字乱码）。
  选了风格后**不输入需求也能直接生成** —— 走该风格的默认 brief；输入了需求则与风格规格合并，冲突时平台硬规则优先、需求中不冲突的部分照做。上传的产品照按「实物保真」约束（不得改设计/配色/材质/件数），二次编辑时改为「编辑范围」约束（只改要求改的地方）。画幅通过 Gemini `image_config.aspect_ratio` 强制，而不只靠提示词。
...
server/                       FastAPI 后端
  ...
  market/                     家具市场决策系统
    ...
    monitor.py                风险与机会信号（21 条月度 + 9 条当月预警，模型只做归纳）
    panels.py                 两个仪表盘的确定性内容（月度层 + 当月实时层）
...
搜索和上传共用一行工具栏。列表每项支持删除，点击后弹窗二次确认；确认后调用登录保护的 `DELETE /api/skills/{skill_id}`，删除对应目录及文件，并移除当前选中状态。内置和导入的 skill 均属于项目共享列表，删除影响所有用户；历史对话保留。取消操作不发送请求，失败则保留列表并显示错误。

内置的竞品定位、新品上架两个 skill 使用“仅卖家精灵”分析模式：聊天与流式接口会在本轮关闭网页、浏览器、知识库及非供应商数据分析路径，先研究再生成内容。无有效取数时返回数据缺口简报；不会用模型知识补数。报告要求逐项证据编号、站点/周期/采集时间、采集值与估算值标识、样本范围和可复算公式；经营建议标为待验证动作，不承诺销量、利润或投放回报。用户信息只作为任务约束。其他自定义 skill 保持原有来源策略。
